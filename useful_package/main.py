...
X_test = np.arage(0.0, 5.0, 0.1)[:, np.newaxis]
...
plt.grid(True)
plt.legend()
plt.title("Random Forest Regression polynom")
mse = mean_squared_error(y_test, y_pred)
print("Mean Squared Error (MSE):", mse)


X_h = np.sort(5 * np.random.rand(90, 1), axis=0)
y_h = hyperbola(X.ravel()) + np.random.normal(0, 1, X.shape[0])
plt.figure(figsize=(8, 6))
...
y_h_test = hyperbola(X_h_test.ravel()) + np.random.normal(0, 1, X_h_test.shape[0])
y_h_test.shape
